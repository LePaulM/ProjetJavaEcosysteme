...
public class Singe extends Charognard{
	...
	public Singe(int dateNaissance, Case emplacement,
		...
		// le singe a accès aux cases de foret
		accesForet = true;

		// le singe a une vitesse de 4
		vitesse = 2;

		// le singe contient 1 viande à la naissance
		viande = 1;
		
		// le singe est marron
		couleur=Color.getHSBColor(21, 46, 35);
								...
								if (c.getAnimal().getAProcree() == false) {						// et enfin si l'autre animal n'a pas déjà procréé récemment 
									...
									for (Case cbis : cases){											// 			on cherche ensuite  
										if (c.getEstVide() == true) {									//			une case vide 
											Animal singe = new Singe (Gestionnaire.getTour(),cbis,
											...
											Gestionnaire.addAnimal(singe);								//			on ajoute l'animal au gestionnaire
